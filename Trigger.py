...
def configure_trigger(cam):
    ...
    try:
        ...
        node_exposure_mode_triggerwidth = node_exposure_mode.GetEntryByName('TriggerWidth')

        if not PySpin.IsAvailable(node_exposure_mode_triggerwidth) or not PySpin.IsReadable(
                node_exposure_mode_triggerwidth):
            print ('Unable to set exposure mode to trigger width (entry retrieval). Aborting...')
            return False

        # Retrieve integer value from entry node
        exposure_mode_triggerwidth = node_exposure_mode_triggerwidth.GetValue()

        # Set integer value from entry node as new value of enumeration node
        node_exposure_mode.SetIntValue(exposure_mode_triggerwidth)
        #print('node_exposure_mode=%s' % node_exposure_mode.GetValue())
        #print('Exposure mode set to trigger width...')
...
def grab_next_image_by_trigger(nodemap, cam,sp):
    """
...
def acquire_images(cam, nodemap, nodemap_tldevice,sp):
    """
    ...
    try:
        ...
        for i in range(NUM_IMAGES):
            try:
                ...
                result &= grab_next_image_by_trigger(nodemap, cam,sp)
                print('hello')
...
def run_single_camera(cam,ncamera,sp):
    """
    ...
    try:
        ...
        if ncamera == 1:
            sp.camera2 = 2
        sp.sendcfg()
        result &= acquire_images(cam, nodemap, nodemap_tldevice,sp)
...
def main():
    ...
    for i, cam in enumerate(cam_list):
        ...
        result &= run_single_camera(cam,i,spim)
